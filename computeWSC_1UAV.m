function [WSC_Val, DeltaComp] = computeWSC_1UAV(A, E, UAVs, dAB, gammaA, gammaJ, channelParam )
%                                 computeWSC_1UAV(A, E, UAV_BM, dAB_R, gammaA, gammaJ, channelParam )
...
%     eta     = (2.^Rs).*(bJ./aJ);
    ...
    aJ = gammaA ./ ( 1 + gammaJ*gJB );
    bJ = gammaA ./ ( 1 + gammaJ*gJE );

    beta = ( (1./gammaA).*( (2.^Rs)-1 ).*(1 + gammaJ.*gJB) );
    eta  = ( (2.^Rs).*(1 + gammaJ.*gJB)./(1 + gammaJ.*gJE) );
